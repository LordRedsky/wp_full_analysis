...
def extract_village_name_from_filename(filename: str) -> str:
    """
    Extract village name from filename like "FORM KENDALI BAHUTARA.xlsx" -> "DESA BAHUTARA"
    """
    import re
    # Look for patterns after "Kendali" or "KENDALI" in the filename
    match = re.search(r'kendali\s+(.+)\.', filename.lower(), re.IGNORECASE)
    if match:
        village = match.group(1).strip().upper()
        # Clean up extra spaces and return with "DESA " prefix
        return f"DESA {village}"
    else:
        # If the pattern isn't found, return a generic name or clean the filename
        basename = os.path.basename(filename)
        # Remove extension and try to extract anything after "kendali"
        clean_name = re.sub(r'\.xlsx$', '', basename, flags=re.IGNORECASE)
        return f"DESA {clean_name.split()[-1]}"  # Use last word as village name


def populate_empty_wp_with_certificate_data(
    wp_file: bytes,
    cert_file: bytes,
    kendali_file: bytes,
) -> bytes:
    """
    Populate empty WP file with certificate data.

    Takes white-colored (unfilled) data from certificate file and fills:
    - NAMA_WP column in WP file with white-colored names from certificate
    - LUAS_BUMI column in WP file with white-colored areas from certificate
    - ALAMAT_OP column in WP file with village name extracted from form control file name
    """
    wp_df, wp_wb = _read_excel_both(wp_file)
    cert_df, cert_wb = _read_excel_both(cert_file)
    ...
    wp_alamat_col = _find_header_col(wp_ws, "ALAMAT_OP")

    cert_nama_col = _find_header_col(cert_ws, "Nama")
    cert_luas_col = _find_header_col(cert_ws, "Luas")

    # Extract village name from kendali filename
    # Try to get the actual kendali filename if it's provided as a path or file object
    kendali_filename = "FORM KENDALI VILLAGE.xlsx"  # fallback

    if isinstance(kendali_file, str):
        # If kendali_file is a file path string
        kendali_filename = os.path.basename(kendali_file)
    elif hasattr(kendali_file, 'name'):
        # If kendali_file is a file-like object with a name attribute
        kendali_filename = os.path.basename(kendali_file.name)
    elif hasattr(kendali_wb, '_filepath'):
        # If we can get the filepath from workbook
        kendali_filename = os.path.basename(kendali_wb._filepath)

    village_name = extract_village_name_from_filename(kendali_filename)

    # Find white-colored rows in certificate and copy to WP
    wp_row_idx = 2  # Start filling from row 2 (after headers)

    for cert_row in range(2, cert_ws.max_row + 1):
        nama_cell = cert_ws.cell(row=cert_row, column=cert_nama_col)

        # Only process if the cell is unfilled (white)
        if _is_unfilled(nama_cell):
            # Get name and luas values
            nama_val = _normalize_str(cert_ws.cell(row=cert_row, column=cert_nama_col).value)
            luas_val = _normalize_str(cert_ws.cell(row=cert_row, column=cert_luas_col).value)

            # Fill WP with certificate data
            wp_ws.cell(row=wp_row_idx, column=wp_name_col, value=nama_val)
            if luas_val:
                luas_int = _to_int(luas_val)
                if luas_int:
                    _set_number(wp_ws, wp_row_idx, wp_luas_col, luas_int)
                else:
                    wp_ws.cell(row=wp_row_idx, column=wp_luas_col, value=luas_val)

            # Fill ALAMAT_OP with village name
            wp_ws.cell(row=wp_row_idx, column=wp_alamat_col, value=village_name)

            wp_row_idx += 1

    # Save the populated WP file
    wp_out = io.BytesIO()
    wp_wb.save(wp_out)
    return wp_out.getvalue()
...
def process_final_analysis(
    ...
    """
    Main function for final analysis.
    
    1. Fill empty WP file with data from certificate file (white-colored rows only)
    2. Perform exact match analysis similar to exact_match.py
    """
    # Step 1: Fill WP file with certificate data
    filled_wp_file = populate_empty_wp_with_certificate_data(
        wajib_pajak_file, 
        sertifikat_file, 
        kendali_file
    )

    # Step 2: Perform exact match analysis on the filled WP file
    wp_df, wp_wb = _read_excel_both(filled_wp_file)
    ...
    # Build mappings for exact matching
    ...
    # Create team output (similar to deep analysis)
    ...
    for r in range(2, wp_ws.max_row + 1):
        ...
        if name_cell.fill and name_cell.fill.start_color and name_cell.fill.start_color.rgb == RED_FILL.start_color.rgb:
            continue
        for c in range(1, wp_ws.max_column + 1):
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 4:
        print("Usage: python final_analysis.py <wp_file> <certificate_file> <kendali_file>")
        sys.exit(1)

    wp_file_path = sys.argv[1]
    cert_file_path = sys.argv[2]
    kendali_file_path = sys.argv[3]

    # Read the input files
    with open(wp_file_path, 'rb') as f:
        wp_bytes = f.read()
    with open(cert_file_path, 'rb') as f:
        cert_bytes = f.read()
    with open(kendali_file_path, 'rb') as f:
        kendali_bytes = f.read()

    # Process the final analysis
    wp_result, team_result, cert_result, stats = process_final_analysis(
        wp_bytes,
        cert_bytes,
        kendali_bytes
    )
    ...
    import os
    base_name_wp = os.path.splitext(wp_file_path)[0] + "_final_result.xlsx"
    base_name_team = os.path.splitext(wp_file_path)[0] + "_team_result.xlsx"
    base_name_cert = os.path.splitext(cert_file_path)[0] + "_final_cert_result.xlsx"

    # Write the results to output files
    with open(base_name_wp, 'wb') as f:
        f.write(wp_result)
    with open(base_name_team, 'wb') as f:
        f.write(team_result)
    with open(base_name_cert, 'wb') as f:
        f.write(cert_result)

    print(f"Final analysis completed successfully!")
    print(f"WP result saved to: {base_name_wp}")
    print(f"Team result saved to: {base_name_team}")
    print(f"Cert result saved to: {base_name_cert}")
